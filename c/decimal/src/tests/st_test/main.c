...
int set_bit_st(int bits, int bit, int new_bit) {
...
s21_decimal str2decimal(const char *number) {
  ...
  for (int i = BITS_IN_INT - 1; i >= 0; i--) {
    if (number[cur_sim_in_num++] - '0')
      res.bits[DECIMAL_INFO] = set_bit_st(res.bits[DECIMAL_INFO], i, 1);
  }

  for (int i = ALL_BIT - 1; i >= 0; i--) {
    if (number[cur_sim_in_num++] - '0')
      *p_get_bits(&res, i) = set_bit_st(get_bits(res, i), i, 1);
  }
