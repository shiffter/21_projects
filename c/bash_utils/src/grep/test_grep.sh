...
echo "Test 39"
    ./s21_grep -f pattern.txt test.txt > s21_grep_res
    grep -f pattern.txt test.txt > grep_res
    DIFF_RES="$(diff -s s21_grep_res grep_res)"
        if [ "$DIFF_RES" == "Files s21_grep_res and grep_res are identical" ]
            then
              (( COUNTER_SUCCESS++ ))
...
echo "Test 40"
    ./s21_grep -f pattern.txt -e option test.txt > s21_grep_res
    grep -f pattern.txt -e option test.txt > grep_res
    DIFF_RES="$(diff -s s21_grep_res grep_res)"
        if [ "$DIFF_RES" == "Files s21_grep_res and grep_res are identical" ]
            then
              (( COUNTER_SUCCESS++ ))
